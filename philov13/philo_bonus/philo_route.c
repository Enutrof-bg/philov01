...
void	*dinner_check(void *data)
{
	...
	while (1)
	{
		...
		while (i < table->nbr_philo
			&& get_int_sem(table->sem_table, &table->end) == 0)
		{
			if (get_int_sem(table->sem_table, &table->all_full)
				== table->nbr_philo)
...
void	*routine(void *data)
{
	...
	increase_int_sem(philo->table_p->sem_table, &philo->table_p->nbr_thread);
	while (get_int_sem(philo->table_p->sem_table,
			&philo->table_p->end) == 0)
	{
		if (philo->full == 1)
		{
			increase_int_sem(philo->table_p->sem_table,
...
void	*philo_solo(void *data)
{
	...
	while (get_int_sem(philo->table_p->sem_table, &philo->table_p->end) == 0)
	{
		if (philo_mort(&philo->table_p->philo[0]) == 1)
		{
			ft_write(philo, DEAD);
			set_int_sem(philo->table_p->sem_table, &philo->table_p->end, 1);
		}
	}
	return (NULL);
}
...
int	start_table(t_table *table)
{
	...
	else if (table->nbr_philo == 1)
	{
		if (pthread_create(&table->philo[0].t1, NULL,
			...
			return (-1);
		set_long_sem(table->sem_table, &table->start, get_time_ms());
